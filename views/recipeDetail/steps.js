...
const Steps = ({navigation, route: {params}}) => {
    ...
    const fetchRecipe= async () => {
        const selectedRecipe = await firestore().collection('recipes').doc(params.recipe.uid).get().then((doc) => {
    ...
    const addNote = async () => {
        ...
        navigation.navigate({name: 'Notes', params: {recipe: params.recipe}})
        } else {
            navigation.navigate({name: 'Notes', params: {recipe: params.recipe}})
        }
}
    ...
    return (
        ...
        <Text>PASOS</Text>
        <Text onPress={() => navigation.navigate({name: 'RecipeDetail', params})}>{params.recipe.name}</Text>
